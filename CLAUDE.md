...
3. **palette/** — `inventory.py` establishes canonical color identities (perceptual clustering, near-white/near-black guards) and is the source of third-party-color records; `fusion.py` (`build_evidence`) accumulates per-`(canonical color, role)` `RoleEvidence` records preserving per-instance salience (σ_i = p_role × π_i, area-driven with bounded position/sibling/contrast modulators); `detect.py` (`detect`) runs the detection-plus-ranking pass: (1) absolute per-`(color, role)` measured salience S_measured, (2) a bounded intent multiplier f = 1 + α·q_intent from declared tokens (re-ranks/rescues at the margin, never vetoes), (3) a two-gate detection test (S_measured ≥ θ_noise AND S_final ≥ θ_present, both absolute), (4) rank survivors by S_final, (5) normalize to display shares last. Divergences (declared-but-unused, used-but-undeclared) fall out of the same pass. All tuning constants (α, per-role λ/β, θ_noise/θ_present, modulator ranges) live in `data/palette_config.yaml` under `detection:`.
