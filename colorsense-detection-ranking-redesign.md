...
> **Metric caveat (separate work item).** The near-extreme switch to CIEDE2000 trusts that metric in a regime where it is itself unreliable — particularly near black, where CIEDE2000 is known to over-report differences between visually indistinguishable colors. The cited near-black case (`#030711` vs `#050505`, CIEDE2000 4.33) may reflect that instability rather than a true perceptual gap. This is orthogonal to the redesign but should be revisited; do not treat CIEDE2000 as ground truth at `L <= 0.15`.
...
6. **Revisit the near-black CIEDE2000 trust** (Section 5.3 caveat) as a separate metric work item.
