...
def _token(
    name: str,
    css: str,
    prior: dict[UsageCategory, float],
    weight: float = 1.0,
...
def _prob_for(results: UsagePalette, category: UsageCategory, css: str) -> float:
    target = _color(css)
...
def test_intent_boost_breaks_tie_toward_declared() -> None:
    ...
    tokens = [
        _token("--accent", a, {UsageCategory.interactive: 0.85, UsageCategory.surface: 0.15}),
        _token("--accent-2", b, {UsageCategory.interactive: 0.15, UsageCategory.surface: 0.85}),
    ]
    posterior, _ = reconcile(usage, tokens, alpha=0.4)

    p_a = _prob_for(posterior, UsageCategory.interactive, a)
    p_b = _prob_for(posterior, UsageCategory.interactive, b)
    assert p_a > p_b
    assert p_a > 0.5
...
def test_declared_but_unused_appears_in_divergence() -> None:
    usage = UsagePalette(
        mapping={
            UsageCategory.interactive: (_entry("#2563eb", 1.0),),
        }
    )
    unused = "#10b981"
    tokens = [
        _token("--primary", unused, {UsageCategory.surface: 1.0}, origin=TokenOrigin.name_rule),
    ]
...
def test_declared_but_unused_gated_to_high_intent_origins() -> None:
    ...
    for low_intent in (TokenOrigin.scale, TokenOrigin.alias, TokenOrigin.fallback):
        tokens = [
            _token("--green-300", unused, {UsageCategory.interactive: 1.0}, origin=low_intent)
        ]
        _, divergence = reconcile(usage, tokens, alpha=0.4)
...
def test_alpha_zero_is_pure_usage() -> None:
    ...
    tokens = [
        _token("--accent", "#10b981", {UsageCategory.interactive: 1.0}, weight=5.0),
    ]
    ...
    p_blue = _prob_for(posterior, UsageCategory.interactive, "#2563eb")
    p_rose = _prob_for(posterior, UsageCategory.interactive, "#e11d48")
...
def test_alpha_one_is_pure_intent() -> None:
    ...
    tokens = [
        _token("--accent", "#e11d48", {UsageCategory.interactive: 1.0}, weight=3.0),
    ]
...
def test_every_category_distribution_normalized() -> None:
    ...
    tokens = [
        _token("--brand", "#2563eb", {UsageCategory.surface: 1.0}),
        _token("--pop", "#e11d48", {UsageCategory.interactive: 1.0}),
    ]
    posterior, _ = reconcile(usage, tokens, alpha=0.4)

    # Every UsageCategory is always present (categories with no entries map to ()); a
    # non-empty category's entry probabilities form a normalized distribution.
    assert set(posterior.mapping) == set(UsageCategory)
...
def test_used_but_undeclared_appears_in_divergence() -> None:
    ...
    assert hits, divergence
    assert hits[0].category == UsageCategory.interactive


def _entry_lists(results: UsagePalette) -> dict[UsageCategory, list[tuple[str, float]]]:
...
def test_alpha_out_of_range_is_clamped() -> None:
    ...
    tokens = [_token("--accent", "#e11d48", {UsageCategory.interactive: 1.0})]
...
def test_near_colors_join_across_usage_and_tokens() -> None:
    ...
    tokens = [_token("--brand-red", declared_red, {UsageCategory.interactive: 1.0})]
...
def test_near_identical_tokens_aggregate_into_one_intent_group() -> None:
    ...
    usage = UsagePalette(mapping={UsageCategory.interactive: (_entry("#10b981", 1.0),)})
    tokens = [
        _token("--a-light-blue", "#2563eb", {UsageCategory.surface: 1.0}, weight=1.0),
        _token("--b-heavy-blue", "#2a66ec", {UsageCategory.surface: 1.0}, weight=3.0),
    ]
    posterior, divergence = reconcile(usage, tokens, alpha=0.4)
    ...
    assert unused[0].category == UsageCategory.surface
...
def test_colors_outside_delta_e_threshold_stay_separate() -> None:
    ...
    tokens = [
        ...
        _token("--green", "#10b981", {UsageCategory.surface: 1.0}),
    ]
    posterior, divergence = reconcile(usage, tokens, alpha=0.4)
